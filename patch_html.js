...
function setupTicketModalAttachments(modal) {
  ...
  // Patch the submit button to upload files after ticket creation
  const origSubmit = submitBtn?.onclick;
  if (submitBtn) {
    submitBtn.addEventListener('click', async () => {
      if (pendingFiles.length === 0) return;
      // Wait briefly for the ticket to be created, then find the newest ticket and upload
      await new Promise(r => setTimeout(r, 800));
      const newest = TICKETS_DATA[TICKETS_DATA.length - 1];
      if (!newest) return;
      for (const file of pendingFiles) {
        try {
          const form = new FormData();
          form.append('file', file, file.name || 'voice-note.webm');
          form.append('ticketId', newest.id);
...
function setupCommentAttachments(container) {
  ...
  // Patch the send button inside the same container or parent
  function findSendBtn() {
    return container.querySelector('button[onclick*="addComment"], button.btn-primary, button[class*="send"]')
      || container.parentElement?.querySelector('button[onclick*="addComment"], button.cmt-send');
  }

  // Poll for send button and patch it
  let patchAttempts = 0;
  const patchInterval = setInterval(() => {
    const sendBtn = findSendBtn();
    if (sendBtn && !sendBtn.dataset.attachPatched) {
      sendBtn.dataset.attachPatched = '1';
      sendBtn.addEventListener('click', async () => {
        if (pendingFiles.length === 0) return;
        await new Promise(r => setTimeout(r, 600));
        // Get the most recent comment for this ticket
        const ticketId = window.currentDetailTicketId || window.currentOpenTicket?.id;
        if (!ticketId) return;
        const cmts = document.querySelectorAll('.cmt-item, .comment-item, [class*="cmt-"]');
        const lastCmt = cmts[cmts.length - 1];
        for (const file of [...pendingFiles]) {
          try {
            const form = new FormData();
            form.append('file', file, file.name || 'voice-note.webm');
...
// Hook into ticket detail open to load attachments
const _origOpenTicketDetail = window.openTicketDetail;
if (typeof _origOpenTicketDetail === 'function') {
  window.openTicketDetail = function(id, ...args) {
    window.currentDetailTicketId = id;
    const r = _origOpenTicketDetail.call(this, id, ...args);
    setTimeout(() => {
      const panel = document.querySelector('.right-panel, .detail-panel, [class*="detail"]');
      if (panel) loadTicketAttachments(id, panel);
    }, 400);
    return r;
  };
}
